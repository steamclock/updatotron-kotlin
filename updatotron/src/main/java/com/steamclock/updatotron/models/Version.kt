...
import com.steamclock.updatotron.UpdatotronException
...
class Version: Comparable<Version> {
    ...
    // Initialization
    //---------------------------------------------------------------------------------
    @Throws(UpdatotronException::class)
    private fun initWith(marketing: String?, buildStr: String?) {
        ...
        if (marketing.isNullOrEmpty()) {
            if (build == null) {
                ...
                throw UpdatotronException.InvalidVersionString()
            } else {
                return
            }
        }
            ...
            marketingComponents = numericMarketing
                ...
                .map { segment ->
                    try {
                        ...
                        throw UpdatotronException.InvalidVersionString(e)
                    }
                }.toIntArray()
        }
    }
